(function( window, undefined ) {
    var YjkJquery = function(selector) {
        return new YjkJquery.prototype.init(selector);
    }
    YjkJquery.prototype = {
        constructor: YjkJquery,
        init: function (selector) {
            // 0.去除字符串两端的空格
            selector = YjkJquery.trim(selector);
            // 1.传入 '' null undefined NaN  0  false, 返回空的jQuery对象
            if(!selector){
                return this;
            }
            // 2.方法处理
            else if(YjkJquery.isFunction(selector)){
                YjkJquery.ready(selector);
            }
            // 3.字符串
            else if(YjkJquery.isString(selector)){
                // 2.1判断是否是代码片段 <a>
                if(YjkJquery.isHTML(selector)){
                    // 1.根据代码片段创建所有的元素
                ...
                // 2.2判断是否是选择器
                    ...
                    // 2.将找到的元素添加到YjkJquery上
                    [].push.apply(this, res);
                }
            }
            // 4.数组
            else if(YjkJquery.isArray(selector)){
                // 转换为真数组
        ...
        eq: function (num) {
            // 没有传递参数
            if(arguments.length === 0){
                return new YjkJquery();
            }else{
                return YjkJquery(this.get(num));
            }
        },
        first: function () {
            return this.eq(0);
    ...
    YjkJquery.extend({
        ...
        isArray : function(sele){
            if(YjkJquery.isObject(sele) &&
                !YjkJquery.isWindow(sele) &&
                "length" in sele){
                return true;
            }
            return false;
        ...
        each: function (obj, fn) {
            ...
            // 2.判断是否是对象
            else if(YjkJquery.isObject(obj)){
                for(var key in obj){
        ...
        clone: function (deep) {
            ...
            if(deep){
                ...
                this.each(function (key, ele) {
                    ...
                    // 遍历元素中的eventsCache对象
                    YjkJquery.each(ele.eventsCache, function (name, array) {
                        // 遍历事件对应的数组
                        YjkJquery.each(array, function (index, method) {
                            // 给复制的元素添加事件
                            $(temp).on(name, method);
                        });
                    });
    ...
    YjkJquery.prototype.extend({
        next: function (sele) {
            ...
            if(arguments.length === 0){
                // 返回所有找到的
                this.each(function (key, value) {
                    var temp = YjkJquery.get_nextsibling(value);
                    if(temp != null){
                        res.push(temp);
                    }
                });
        ...
        prev: function (sele) {
            ...
            if(arguments.length === 0){
                this.each(function (key, value) {
                    var temp = YjkJquery.get_previoussibling(value);
    ...
    YjkJquery.prototype.extend({
        attr: function (attr, value) {
            ...
            // 2.判断是否是对象
            else if(YjkJquery.isObject(attr)){
                var $this = this;
                // 遍历取出所有属性节点的名称和对应的值
        ...
        css: function (attr, value) {
            ...
            if(YjkJquery.isString(attr)){
                // 判断是一个字符串还是两个字符串
                if(arguments.length === 1){
                    return YjkJquery.getStyle(this[0], attr);
                }else{
                    this.each(function (key, ele) {
    ...
    YjkJquery.prototype.extend({
        on: function (name, callBack) {
            ...
            this.each(function (key, ele) {
                ...
                if(!ele.eventsCache[name]){
                    ...
                    // 5.添加对应类型的事件
                    YjkJquery.addEvent(ele, name, function () {
                        YjkJquery.each(ele.eventsCache[name], function (k, method) {
                            method.call(ele);
                        });
                    });
                }else{
        ...
        off: function (name, callBack) {
            ...
            // 3.判断是否传入了两个参数
                ...
                this.each(function (key, ele) {
                    YjkJquery.each(ele.eventsCache[name], function (index, method) {
    ...
    YjkJquery.prototype.init.prototype = YjkJquery.prototype;
    window.YjkJquery = window.$ = YjkJquery;
})( window );